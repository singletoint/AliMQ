...
namespace DBPresistence
{
    public class SysMessageQueueDbSvc : BaseDbSvc
    {
        ...
        public List<SysMessageQueue> ListByKeyList(List<string> keys)
        {
            using (EnsureUserConnection())
            {
                return (DbConnection.Query<SysMessageQueue>(@"Select * From SysMessageQueue Where Keys In @Keys", new { Keys = keys })).ToList();
            }
        }
